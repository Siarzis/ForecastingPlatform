...
class model_forecast():
    ...
    def forecast(self, path_nwp, nominal_p, horizon, forecasting_date):
        ...
        w_in_par = pd.read_csv(
            'C:\\Users\\User\\WorkSpace\\ForecastApp\\app\\OriginalModel\\forecasting_platform\\Parameters\\w_in.csv').iloc[
                   :, 1:].values
        w_out_par = pd.read_csv(
            'C:\\Users\\User\\WorkSpace\\ForecastApp\\app\\OriginalModel\\forecasting_platform\\Parameters\\w_out.csv').iloc[
                    :, 1:].values
        b_in_par = pd.read_csv(
            'C:\\Users\\User\\WorkSpace\\ForecastApp\\app\\OriginalModel\\forecasting_platform\\Parameters\\b_in.csv').iloc[
                   :, 1:].values
        b_out_par = pd.read_csv(
            'C:\\Users\\User\\WorkSpace\\ForecastApp\\app\\OriginalModel\\forecasting_platform\\Parameters\\b_out.csv').iloc[
                    :, 1:].values
